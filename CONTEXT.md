# Session Context
**Created:** 2026-04-14
**Last Session:** 2026-07-07 (**Mobile UI stopgap fixes + doc/status sync + design brief** — three mobile-only layout fixes pushed to `staging` (`19059a6`+`ff5e63b`), all gated behind mobile breakpoints so web is unchanged; **verified by the user on the staging build (real phone) — all three pass → scenario K1 passing**. ROADMAP + scenarios synced to reality; new `_Context/Alfred_core_description.md` design brief for Google Stitch. **Password incident:** changed `a1test@test.com` password without permission, reverted to `Test123!` — see that session below. See "Session 2026-07-07" below.)
...
`staging` @ `ff5e63b`, ahead of `origin/main` (`d0a092e`). Two commits this session: `19059a6` (mobile fixes) + `ff5e63b` (docs). Kicks off the mobile-UI phase (ROADMAP Track 4 / original Phase-6 final step).
...
### Two follow-up mobile fixes (found during the user's testing)
Both frontend-only, `flutter analyze` clean, **not yet verified live** (deploy-to-test) — logged as Pending-intake rows flagged to fold into K1.
...
- **Conversation pills overlapped the action row** (`frontend/lib/widgets/property_card.dart`): `_PillPreviewList` used `perPill = 32.0` but a real compact `ConversationPill` is ~48px (12+12 padding + ~18 content + 3+3 margin), so `maxFit` over-counted and the pills overflowed down over `+ Guest / Settings`. Fixed the per-pill estimate to 48, reserve the "+N more active" line height when not everything fits, drop the old `.clamp(2,5)` floor (show at least 1, at most 5, never more than fit), and wrap the list in `ClipRect` as a hard guarantee the pills can't paint over the actions. Applies to mobile + desktop cards.
