...
| 2026-09-19 | staging (this session, not yet pushed) | Stop leaves an orphaned "ghost" property if the host leaves without retrying | Design gap found live while verifying D8: `cancel_initial_ingest_run` clears `status`/`ingest_run_id` but never sets `deleted_at`, so if the host clicks Stop then navigates away (back button, close tab) instead of hitting Train Now again, the property row persists forever with a null status — the dashboard has no filter for this and no dedicated UI for it, so it likely renders via `_StatusBadge`'s generic fallback with a "Details" action pointing at a near-empty drawer. Needs a founder decision: auto-delete on Stop instead (simpler, matches "nothing was really created yet"), or keep it as a resumable draft with real UI for that state (bigger scope). Not yet decided or fixed. | New — no related row yet |
