...
- **id:** dashboard-delete-processing-01
- **touches:** `frontend/lib/widgets/property_card.dart`, `frontend/lib/screens/dashboard_screen.dart`, `backend/services/supabase_client.py`
- **layer:** 2 — `_tests/runner/scenarios/d9.ts`
- **setup:** a property actually in a Processing status (Ingesting/Training/Merging) on the dashboard
- **action:** tap the x next to "Processing…", confirm "Delete Forever" on the resulting dialog
- **host_expected:** the card disappears from the dashboard (same realtime-or-10s-polling mechanism D5 already covers — confirmed live it can take close to that full 10s, not just realtime's usual latency)
- **db_expected:** `deleted_at` set, same soft-delete as the drawer's existing "Delete Forever" — the property must be re-added from scratch, no rollback
- **last_tested:** 2026-09-19 — PASS
- **status:** passing
...
|---|---|---|---|---|
| 2026-09-25 | staging `4bcdbb9` (pushed, deployed) | Sentry error tracking added across backend, scraper, frontend -- infra/observability, not a user-facing flow | 3 new Sentry projects (`alfred-backend`/`alfred-scraper`/`alfred-frontend`, org `alonso-vazquez-ng`), SDK bootstrap gated on empty `SENTRY_DSN` = off, plus explicit `capture_exception()` at every pre-existing except block that only logged and swallowed a real failure (the 2026-09-21 resolver `call_timeout` incident's own failure pattern). **Fully live-verified, not deferred**: real `capture_message()` calls for backend/scraper confirmed as landed issues via the Sentry MCP (event IDs matched server-side); frontend verified via a real Playwright-triggered uncaught error (temporary URL-gated `?sentry_verify=1` trigger, removed before commit) against the actual built app, confirmed as a landed issue server-side too -- proves `SentryFlutter.init`'s automatic zone-based capture fires for real, not just present in the bundle. `SENTRY_DSN`/`ENVIRONMENT` set on all 4 Cloud Run services; `alfred-backend-staging`/`alfred-scraper-staging` redeployed with the new code and health-checked (200) post-deploy. Not yet done: prod Cloud Run deploy (needs its own manual `gcloud run deploy --source` after a `staging->main` merge, no auto-deploy on this project) and a real triggered-error check against the *deployed* staging URLs specifically (verification above used the local build + real DSNs, not the live Cloud Run/Vercel instances). | Not grouped -- infra rollout, not a reusable user-facing scenario |
...
| 2026-09-21 | staging (this session, not yet pushed) | Add Property first-run -- two real, distinct causes of "popup stacked on another popup" found and fixed (supersedes the two rows above's "not yet live-tested" status -- founder's live retest after those two shipped found this new bug on top) | Founder's exact repro: Train Now -> wait popup -> conflicts-found popup appears stacked on the wait popup -> clicking Resolve Conflicts dismisses the front popup but the wait popup ("Alfred is learning your property...") is still there underneath, staying until "Continue in background" is manually clicked -- not a brief animation overlap, permanent until manual dismissal. Cause 1: `add_property_screen.dart`'s `_startIngest` closed the wait dialog via a blind `Navigator.pop()` (closes whatever is CURRENTLY topmost) instead of a route-keyed close -- if anything else had been pushed on top by the time this ran, the blind pop closed that instead, leaving the wait dialog stuck. Fixed: switched to the existing `pushTrainingWaitDialog`/`popTrainingWaitDialog` route-keyed helpers (`training_wait_dialog.dart`, already used by `edit_property_screen.dart`/`property_detail_drawer.dart` for this exact class of bug), storing the route and `await`ing its real removal before showing the deferred result dialog. Cause 2 (the thing that was landing on top and causing cause 1's pop to miss): `dashboard_screen.dart` never actually stops running while Add Property screen is open on top of it (just pushed as another route) -- its own background `_checkTrainingCompletion`/`_checkScrapeRetryResolved` (10s poll + realtime) has zero exclusion for a property currently being created there, despite the class-level comment already claiming first-time creation "already handles its own popup locally" (that exclusion was never actually coded). So Dashboard independently fires its own competing popup for the exact same status transition, landing on the same root navigator on top of Add Property's own popup, with zero coordination between the two. Fixed: `AddPropertyScreen` gained an `onPropertyIdKnown` callback (fired once in `initState` with its client-generated id, and again if the backend resolves a different id via the rename path); `dashboard_screen.dart`'s `_openAddProperty` stores it in a new `_activeAddPropertyId` field (cleared once that screen closes) and both completion-check functions skip that id. Also added in the same pass: Add Property screen now auto-scrolls to the conflicts section (`Scrollable.ensureVisible` on a new `_conflictSectionKey`, same pattern already applied to `edit_property_screen.dart` last session) when status transitions to `Conflict_Pending` -- founder's other still-open complaint from the same retest ("have to scroll down manually"). `flutter analyze` clean (0 new issues). Not yet live-tested against a real first-time Train Now run with a real conflict on deployed staging -- needs the exact repro sequence re-run post-deploy. | Not grouped -- new root cause, distinct from the routing/delay fixes above |
| 2026-09-21 | staging (this session, not yet pushed) | guide.html -- Property Enhancement (Overview/Knowledge) and Add a Guest (all 9 steps) screenshots replaced with real captures of the actual in-app walkthroughs, instead of a hand-drawn CSS approximation of the highlight | Founder's repeated complaint: the guide's highlight boxes used the wrong color and covered the wrong areas because they were reverse-engineered from a static image instead of using the real `WalkthroughHighlight` UI directly. Fixed by driving the real walkthroughs live via Playwright (project's own `env.ts`/`loginAs`, no new credentials) against the deployed staging property drawer, guest-link dialog, and Host Chat dialog, capturing a cropped screenshot at each real step (the real purple highlight box is baked into the pixels now, not redrawn), and splicing them into `guide.html` via a one-off script (base64-embedded, same as the page's existing images). Overview's single 2-highlight screenshot split into one real screenshot (Manage button highlighted); Knowledge's single 2-highlight screenshot split into two real screenshots (Add New Knowledge / Automated Learning, matching the real walkthrough's own two separate steps -- can't show both highlighted at once for real, so the guide no longer pretends it can). Add a Guest steps 2-9 (previously text-only, no screenshots at all) each got a real screenshot from the live Host Chat walkthrough, including the escalation-demo and resolved states. Also fixed while live-capturing: step 7's copy was missing the real "hit Send first" instruction before Mark Issue as Resolved (the real walkthrough requires it, the guide's old copy didn't mention it). The Add Property tab's remaining 3 CSS-drawn highlight boxes (sign-in Sign-up link, Airbnb URL field, dropzone) don't have an easily-reachable real walkthrough equivalent (only triggers on an empty-properties account) -- left as CSS boxes but corrected to the real app's brand purple (`#6E38A7`, was reusing the wrong generic `#6366f1`). Verified visually via local `file://` Playwright render after the splice (all 12 real screenshots render correctly with the highlight visible in the right place, bold copy intact) -- not yet re-verified against the deployed staging URL per the founder's standing instruction to check the real deploy, not just the local file. | Not grouped -- doc-only, no app code touched |
| 2026-09-19 | staging `b2b5617` (pushed) | Add Property form -- training tips split into "Gold for Alfred" / "Also helps" tiers | Both the docked walkthrough panel and the form's own tips card now render two labeled, colour-coded groups (warning/gold for Gold tips, success/mint for Also-helps) instead of one flat list -- matches the same restructure in guide.html's Step 3. Verified via a live Playwright screenshot against staging after deploy: both boxes render with correct labels/colors/content, `flutter analyze` clean. Not yet exercised as a real Playwright scenario (just a one-off capture script, not committed) -- the walkthrough-panel's own copy of this list (shown during onboarding, gated behind the guest-link/post-training "seen" flags) is unverified live since the capture script deliberately pre-seeded those flags to suppress it. | Group with the guide.html Step 3 rewrite (same content, no code, doc-only, exempt from its own row) |
| 2026-09-19 | staging `8cb177c` (pushed) | Ghost-property recovery UX -- FIXED, decided and shipped (supersedes the row this replaces) | Founder decided: "Training incomplete" status + two ways out (Resume or delete), not auto-delete. Built on top of the same-day run_start fencing fix above -- reused existing infrastructure rather than inventing new state: `_StatusBadge`/`_buildActions` (`property_card.dart`) recognize `status.isEmpty && !wasTrainedBefore` as a distinct "Training incomplete" card state (amber badge, "Resume Training" action + a red delete x, reusing the existing soft-delete confirm dialog); `setup_status.dart`'s `nextStepFor` gained a matching case so the *existing* `SetupStatusBanner` (same banner style used everywhere else in the app) shows it in both the drawer's Overview tab and Edit Property, no new UI component; `edit_property_screen.dart`'s `_handleNextStepAction` routes it to the same `_startIngest()` every other resume path already uses. Live-verified end-to-end on staging: stopped a real first-run, confirmed the dashboard card showed "Training incomplete"/Resume Training/x, opened the drawer and confirmed the same banner + preserved Airbnb URL + "No files yet", clicked through to Edit Property and confirmed the identical banner there, clicked its Resume Training button and confirmed `/api/ingest` actually re-dispatched (real network capture, not just a UI check) and the standard training-wait dialog appeared. | Group with the run_start fencing fix (same root cause, same day) |
...
| 2026-09-19 | staging (this session, not yet pushed) | Dashboard Step 0 hint card overlay — supersedes the row above's open caveat | Founder live-tested with two real Ready properties and found a real bug: the account-wide "seen" flag itself worked correctly (global, not per-property, as intended), but `_showStep0Hint` in `dashboard_screen.dart` gated only on status + seen-flags — with both properties Ready and unseen, the Step 0 overlay rendered on *every* Ready card instead of just one. Fixed: added a check that the card is the *first* Ready property in `_properties` (dashboard render order), so the hint now shows on exactly one card at a time regardless of how many Ready properties exist; re-toggling "Show walkthrough again" in Settings replays it on that same first card, matching the founder's stated intent. `flutter analyze` clean. Not yet live-verified on deployed staging (fixed same session, pending push). | Group with D6 (same walkthrough surface, same `dashboard_screen.dart`/`walkthrough_prefs.dart` area) |
