...
| 2026-07-16 | d2444b1, a0c1fdc + UptimeRobot | Telegram / web-search reply | A guest question needing the **grounded web-search 2nd pass** returns a proper reply within `GEMINI_TIMEOUT_S` (45s) — no `504`. ROOT CAUSE (confirmed by the new per-attempt logging): `min=0` cold-start Vertex dynamic-quota **429s** stacking on the old **2/4/8s** backoff. FIX: per-attempt retry logging (`d2444b1`) + **shorter jittered backoff ~0.5/1/2s** (`a0c1fdc`) + **UptimeRobot keep-warm** ping on staging `/health` (no cold starts); prod already `min=1`. Verified: 3 back-to-back local-rec queries → all proper replies, only 1 cold 429 (recovered on retry). | telegram-timeout |
| 2026-07-16 | 01033c1 | Scraper / ingest resilience | An Airbnb scrape+structure survives a transient Vertex 429 (dynamic shared quota) instead of failing the ingest — `scraper/main.py` now retries the Gemini call with jittered backoff, matching the backend (was the one raw `generate_content` with no retry). | scraper-429 |
| 2026-07-16 | infra (Cloud Build) | CI / prod deploy | Pushing to `main` auto-builds+deploys backend+scraper to prod Cloud Run via trigger `deploy-prod-on-main` (europe-west3, runs as compute SA, `cloudbuild.yaml` code-only — env/secrets preserved). First real fire = next `staging→main` merge. | ci-trigger |
| 2026-07-16 | infra (Vercel) | Staging frontend rename | Staging frontend is reachable and fully functional at the new domain **`alwaysalfred-staging.vercel.app`** (Production, same deployment/env as before — verified via `/assets/.env`). Old domain `alfred-ingestor.vercel.app` **307-redirects** there (not deleted). Web guest chat + host dashboard both load correctly from the new domain; `FRONTEND_URL` on `alfred-backend-staging` (rev 00005) includes the new domain first (guest links) plus both old domains (CORS). | vercel-rename |
