...
| 2026-09-19 | staging `41215ab` (pushed) | Stop leaves an orphaned "ghost" property if the host leaves without retrying | Design gap found live while verifying D8: `cancel_initial_ingest_run` clears `status`/`ingest_run_id` but never sets `deleted_at`, so if the host clicks Stop then navigates away (back button, close tab) instead of hitting Train Now again, the property row persists forever with a null status. **Confirmed by code trace (2026-09-19), not just a guess:** `dashboard_screen.dart`'s fetch filters only on `deleted_at is null`, not status, so the row *does* render as a real card -- a grey "Unknown" `_StatusBadge`, and `_buildActions` falls through every branch (not processing/error/conflict/ready) to the final generic "Details" button, which opens a near-empty drawer (blank status line, the preserved Airbnb URL, no resume-training path). A real, visible dead-end card, not an invisible orphan. Needs a founder decision: auto-delete on Stop instead (simpler, matches "nothing was really created yet"), or keep it as a resumable draft with real UI for that state (bigger scope). Not yet decided or fixed. | New — no related row yet |
