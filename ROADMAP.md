...
- **M1 — Cutover steps remaining (status 2026-07-14):** Gate 2 is **13/15** green; the two open rows are **P-1** (confirm-email link retest, the flow changed) and **P-3** (the >15 MB **ingest** cap — never actually tested; the earlier attempt hit the *guest-chat* 10 MB limit, a different one). Also awaiting founder sign-off: **A7** (delete account) + **M1/M2** (voice). Then the flip, in strict order: **disable Render-prod `autoDeploy` FIRST → merge `staging→main` → tag `v1.0.0-beta.1` → point Vercel `alwaysalfred` at `main` → keep a pre-prod frontend (repoint `alfred-ingestor` at `staging`+staging DB, or you'll have none) → retire Render prod** → staging→Cloud Run (below) → CI/CD trigger (below). **Nothing is merged yet.**
...
- 🔨 **M1 — Web voice recorder (2026-07-14, `d28cb44` + `72ba4c2`, AWAITING FOUNDER RETEST).** The ~30% truncation took **two** attempts. `record_web` resamples (lossily) unless the requested rate equals the mic's real rate. Attempt 1 requested `AudioContext().sampleRate` — the **output** device — which on a 44.1kHz-output/48kHz-mic machine still asked for a downsample (`48000/44100 = 1.088`, i.e. the ~9% still seen: 9/10s, 8/10s). Attempt 2 reads the **mic track's** own rate, reproducing `record_web`'s internal `_adjustContext` exactly; added `_auditWav()` which logs real duration vs wall clock so a residual loss self-reports. **UX shipped:** mic → glowing red **Stop** (in the mic's position) → **review** bar (discard / send), a live `m:ss` counter, and a **60s cap** that stops into review (never auto-sends). Scenarios **M1/M2**, marked `failing` until the founder confirms. ⚠️ Keep WAV (Gemini rejects `audio/webm`); `numChannels:1` is load-bearing.
