...
- 🔴 **M1 — Web voice recorder (from the 2026-07-14 verification pass, NOT yet fixed):** recordings are **truncated ~30%** (root cause + one-line fix documented in `lessons.md` + the `_tests/scenarios.md` intake row). Also missing: a **live duration counter**, a **60s cap**, and an obvious/glowing recording indicator.
...
- 🔨 **M1 — "Delete account" (founder-specified 2026-07-14, NOT yet built).** Lives in the **profile menu**. Deletes the host's account + all property data, but **keeps the conversations archived under a pseudonymized / anonymous name** rather than destroying them. Needs: a destructive-action confirmation, a backend endpoint (service-role), and a decision on what "pseudonymized" means for `guests.name` + the `learning_events` ledger — so it lands with D4, not before it.
