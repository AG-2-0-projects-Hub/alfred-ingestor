...
- 🔴 **M1→M2 — WhatsApp live (DECIDED 2026-07-20 — WA-first for the MX/LATAM beta; next dedicated session):** implement WhatsApp as a native channel, **pulled forward ahead of beta** because WA is the dominant channel in Mexico and it keeps the host's **personal number private** (guests talk to Alfred's number; the host needs no number of their own — no second SIM). **Decisions:** (1) **one shared Alfred/HostWhisperer WA number** for all hosts, booking-code linking via a `wa.me` deep link (mirrors Telegram's `/start booking_id`); (2) **Meta Cloud API direct** (official, webhook-based) — a **port of the native Telegram integration** into the existing `active_channel` model as `active_channel='whatsapp'`, same `webhook → Cloud Tasks → process → reply` shape, same Brain + multimodal path. **Cost:** guest-initiated chat is **free** — replies land inside Meta's 24h service window; only proactive *out-of-window* messages cost (~$0.008 MX utility), which also de-risks **R3**. **Setup (~3–10 business days, founder-gated — START NOW in parallel):** a brand-new **never-on-WhatsApp** number (must receive SMS/voice) + Meta Business Account → WhatsApp Manager → WABA → phone-number registration → Meta App + System User token; **full Meta Business Verification is NOT needed for beta** (an unverified WABA's ~250 conversations/day is plenty for 20–30 properties; verify in parallel — ties to **D8**). Unified conversation preserved; per-channel *sub-guest* view filtering still deferred (see channel-isolation note above).
...
| D2 | WhatsApp channel path | Beta channel (pulled to WA-first) | 🔨 **DECIDED 2026-07-20** — **shared Alfred number + Meta Cloud API direct**, pulled forward as WA-first for the MX beta. Founder to procure a **fresh never-on-WhatsApp number** + start the Meta Business Account/WABA setup (~3–10 biz days). Full Business Verification **deferred** (unverified WABA fine for beta). Dedicated build session next. |
