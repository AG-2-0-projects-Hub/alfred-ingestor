...
- **M1 — Cutover status (2026-07-15): merge gate is CLEAR.** **Gate 2 = 15/15** (P-1 + P-3 closed by the founder 2026-07-15); A7 (delete account) + M1/M2 (voice) also signed off; both prod SQL runs (RLS migration + Gate-2 test-data cleanup) done. **Remaining = the flip itself**, in strict order: **founder disables Render-prod `autoDeploy` FIRST → merge `staging→main` → tag `v1.0.0-beta.1` → founder points Vercel `alwaysalfred` at `main` → founder repoints `alfred-ingestor` at `staging`+staging DB (pre-prod frontend, else none) → smoke test → retire Render prod (delete `render.yaml`)** → staging→Cloud Run (below) → CI/CD trigger (below). **Nothing is merged yet — this is the next session.**
