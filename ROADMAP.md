...
**Version:** 1.4
**Created:** 2026-06-30 · **Updated:** 2026-07-16 (**THE FLIP shipped** — merged `staging→main` (PR #3), tagged **`v1.0.0-beta.1`**, prod on `main`, old Render prod retired; **staging moved to Cloud Run** (platform parity); **Cloud Build trigger pending** the founder's GitHub↔Cloud Build OAuth; env-parity probe caught + fixed a live prod RLS gap; a **Telegram-504 timeout** bug is open. See `_Context/session-digest.md`.) · Prior: 2026-07-12 (**infra cutover executed** — DB split into a fresh prod Supabase project + backend/scraper migrated to **Cloud Run**, prod isolated on its own GCP project / Telegram bot / Gemini key; full multimodal (Alfred sees images + hears voice) shipped. Gate 1 (staging) **passed**; Gate 2 (new-prod e2e) pending → then `staging→main`. Two new 🔴 M1 items added: **staging/prod platform parity** + **CI/CD trigger on `main`**. See `_Context/session-digest.md`)
...
- 🔨 **M1 — CI/CD for prod (in progress 2026-07-16):** **`cloudbuild.yaml` committed** (`72e2edc`) — on push to `main`, builds + deploys backend + scraper to prod Cloud Run, **code-only** (no `--set-env-vars`, so prod env/secrets are preserved). **Remaining:** founder completes the **GitHub↔Cloud Build OAuth** (Repositories 2nd-gen, connection e.g. `github-alfred-ingestor`) → Claude grants the compute SA `roles/run.admin` + `roles/iam.serviceAccountUser` and creates the trigger (branch `^main$`). First fire = the next `staging→main` merge. Cloud Run does **not** auto-deploy on merge (initial deploys were manual `gcloud run deploy --source`), so until this trigger exists, merging `staging→main` will *not* update prod. Conversely, `render.yaml` has `autoDeploy: true` — **disable Render-prod autoDeploy before the merge**, or it will redeploy the old prod stack against the old DB.
