...
## 2026-07-20 — WhatsApp channel build: a secret-naming trap, a shared-queue trap, and a regex that nearly ate "check-out"

**Context:** Building the WhatsApp guest channel as a port of the native Telegram channel (ROADMAP D2), deployed to staging (rev 00006) the same session.

**Discovery:**
1. **Unprefixed Secret Manager names silently mean PROD in this project — and it is easy to violate without noticing.** The convention (`TELEGRAM_BOT_TOKEN` = prod, `staging-telegram-bot-token` = staging) is nowhere enforced by tooling; it is tribal knowledge. The founder saved the WhatsApp **test-WABA** credentials as unprefixed `WHATSAPP_ACCESS_TOKEN` / `WHATSAPP_APP_SECRET` while following a generic Meta setup guide that had no reason to know the convention. Left alone, a future prod deploy would have read them and silently sent from a test number reachable by only 5 allowlisted people — **green in staging, dead for real guests, no error anywhere** to surface it. Caught only because the values were double-checked before wiring them into the Cloud Run deploy. **Global Candidate: Yes** — any AG project using an unprefixed/prefixed convention for env-scoped secrets should either enforce it in tooling (e.g. `ag-switch` or a pre-deploy lint) or document it somewhere louder than a comment in `CONTEXT.md`.
2. **One shared Cloud Tasks queue silently starves a second channel added later.** `task_queue.enqueue` read a single `CLOUD_TASKS_QUEUE` env var. Adding WhatsApp without threading a queue override through would have dispatched its tasks onto `telegram-updates-staging` — the new `whatsapp-updates-staging` queue would sit empty and unused, one channel's retry storm would delay the other's replies, and the Cloud Console would show the wrong queue as busy. Fixed with an optional `queue=` parameter on `enqueue()` plus a per-channel env var (`WHATSAPP_TASKS_QUEUE`), falling back to the original var when unset. **Global Candidate: Yes** — any shared dispatch helper that gains a second caller needs to ask "does this caller need its own queue/topic/rate-limit," not just "does it need its own task name."
3. **A booking-code recognition regex that looked obviously right matched "check-out."** WhatsApp has no equivalent of Telegram's invisible `/start` payload — the booking id has to ride inside an editable, prefilled `wa.me?text=` message, so inbound text has to be scanned for something that looks like a booking id. The natural first regex, `[a-z0-9]+(-[a-z0-9]+)+`, matches any hyphenated lowercase phrase — including "check-out," "check-in," "wi-fi," and "self-service," all of which guests type constantly in this domain. Shipped, it would have diverted ordinary conversation down the linking/lookup path instead of the burst-coalescing path on every such message. Fixed by requiring the final hyphenated segment be exactly 6 characters (the real generator is `<slug>-<6 random alnum>`), then proved it against both directions: 2,100 freshly generated ids all matched, and a list of common domain phrases all failed to match. **Global Candidate: Yes** — any regex meant to recognize a generated-id pattern inside free-form user text needs a companion test asserting realistic non-matches, not just matches; matching is the easy half.
4. **WSL had neither `python3-venv` nor `pip`, and `ensurepip` was also absent — `sudo` needing an interactive password made the usual fix impossible mid-session.** Installing `python3-pip` via apt needs root; asking for a password non-interactively is a dead end (and shouldn't be worked around). The unblock: pip ships a **standalone zipapp** (`https://bootstrap.pypa.io/pip/pip.pyz`) that runs directly via `python3 pip.pyz install --target <dir> ...` with no system changes at all — no venv, no root, no PEP-668 externally-managed-environment block. Used to install the backend's requirements into `/tmp/wa-deps` for running `_tests/whatsapp_channel.py` and driving the FastAPI app via `TestClient` before ever deploying. `/tmp` does not survive a reboot; re-run the same command to rebuild it.
5. **`gcloud` is not on `$PATH` in this WSL shell, and the fix everyone reaches for (`export PATH=$PATH:~/google-cloud-sdk/bin`) fails with a bash syntax error** — the inherited `$PATH` carries Windows entries like `/mnt/c/Program Files (x86)/...` whose spaces and parentheses break unquoted expansion, and `~` does not expand inside a quoted assignment either. The reliable fix is calling gcloud by its absolute path every time: `~/google-cloud-sdk/bin/gcloud ...`. (Also saved to Claude's cross-session memory, since it recurs in every WSL shell regardless of project.)
6. **WhatsApp's "free" service-message window is not permanently free.** Meta's own pricing docs state per-message billing extends to service (guest-initiated) messages effective **2026-10-01** — this project's cost model (and D2's original "guest chat is free" framing) was costed on conversation-based pricing that predates that change. The 24h *rule* governing when a free-form send is allowed is unaffected; only the *cost* of using it changes. Flagged in `ROADMAP.md` D2 and the cost-telemetry backlog item for re-baselining before that date.
